...
int TESTSPEED = 11; //テスト用の速さ（一律）

int LANE_SPEED_R = 17; /* 右レーンチェンジでの速度 */
int LANE_SPEED_L= 14; /* 左レーンチェンジでの速度 */
int CRANK_SPEED=  17; /* クランク中の速度 */

#define     RUNNING_TIME        31000L  /* 走行時間(ms)                 */
//#define    ENC_END       1200L*(dipsw_get()+1)/* 走行距離(cm) */
  ...
  while ( 1 ) {
    ...
    switch ( pattern ) {
      ...
      case 64: // (右)車線変更処理 4
        ...
        if ( (lEncoderTotal - lEncoderCrank) >= (109L * 2) ) { // 10mm で 3 150mm 後に通常トレースへ遷移
          lEncoderCrank = lEncoderTotal;
          causeout = lEncoderTotal;
          pattern = 11;
          break;
